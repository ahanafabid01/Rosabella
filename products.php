                ...
                <aside class="sidebar-desktop">
                    ...
                    <div style="margin-bottom: 2rem;">
                        ...
                        <ul style="display: flex; flex-direction: column; gap: 0.5rem;">
                            <li>
                                <a href="products.php" style="display: flex; justify-content: space-between; padding: 0.5rem 0; color: <?= !$category ? 'var(--color-primary)' : 'var(--color-text-light)' ?>;">
                                    <span>All Products</span>
                                </a>
                            </li>
                            <?php foreach ($categories as $cat): ?>
                                <li>
                                    <a href="products.php?category=<?= urlencode($cat['slug']) ?>" style="display: flex; justify-content: space-between; padding: 0.5rem 0; color: <?= $category === $cat['slug'] ? 'var(--color-primary)' : 'var(--color-text-light)' ?>;">
                ...
                <div>
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 1rem; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; padding-bottom: 1rem; border-bottom: 1px solid var(--color-border);">
                        <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                            <a href="products.php?<?= http_build_query(array_merge($_GET, ['filter' => ''])) ?>" class="btn <?= !$filter ? 'btn-primary' : 'btn-secondary' ?> btn-sm">All</a>
                            <a href="products.php?<?= http_build_query(array_merge($_GET, ['filter' => 'new'])) ?>" class="btn <?= $filter === 'new' ? 'btn-primary' : 'btn-secondary' ?> btn-sm">New</a>
                            <a href="products.php?<?= http_build_query(array_merge($_GET, ['filter' => 'bestseller'])) ?>" class="btn <?= $filter === 'bestseller' ? 'btn-primary' : 'btn-secondary' ?> btn-sm">Best Sellers</a>
                            <a href="products.php?<?= http_build_query(array_merge($_GET, ['filter' => 'sale'])) ?>" class="btn <?= $filter === 'sale' ? 'btn-primary' : 'btn-secondary' ?> btn-sm">On Sale</a>
                        </div>
                        
                        <div style="display: flex; align-items: center; gap: 0.5rem;">
                            <label style="font-size: 0.875rem; color: var(--color-text-light);">Sort by:</label>
                            <select class="form-select" style="width: auto;" onchange="window.location.href='products.php?<?= http_build_query(array_diff_key($_GET, ['sort' => ''])) ?>&sort=' + this.value">
                    ...
                    <?php else: ?>
                        ...
                        <?php if ($totalPages > 1): ?>
                            <div style="display: flex; justify-content: center; gap: 0.5rem; margin-top: 2rem;">
                                <?php if ($page > 1): ?>
                                    <a href="products.php?<?= http_build_query(array_merge($_GET, ['page' => $page - 1])) ?>" class="btn btn-secondary">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                ...
                                <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
                                    <a href="products.php?<?= http_build_query(array_merge($_GET, ['page' => $i])) ?>" class="btn <?= $i === $page ? 'btn-primary' : 'btn-secondary' ?>"><?= $i ?></a>
                                <?php endfor; ?>
